...
def ingest_docs(connector, embedder, store, tracker, chunk_size, chunk_overlap):
    ...
    for doc in docs:
        ...
        fingerprint = content_fingerprint(doc.content)
...
def ingest_issues_and_prs(connector, embedder, store, tracker, chunk_size, chunk_overlap):
    ...
    for item in all_items:
        ...
        fingerprint = item.last_modified or content_fingerprint(item.body)
...
def run_github_docs_ingestion(config: dict, tracker: IngestionTracker, embedder: Embedder, store: VectorStore) -> dict:
    """Run GitHub docs, issues, and PRs ingestion. Returns stats dict."""
    ...
    return {
        ...
        "skipped": doc_skipped + issue_skipped,
        "errors": doc_errors + issue_errors,
    }
...
def main():
    ...
    log_with_data(
        logger, logging.INFO,
        f"Done: {stats['chunks']} ingested, {stats['skipped']} skipped, {stats['errors']} errors ({elapsed:.1f}s)",
        source="github_docs", **stats, elapsed_seconds=round(elapsed, 1),
    )
